...
{/if}<br><br>
            ...
            <ul id="limitations" class="sub">
               {foreach from=$limitations key=l_id item=l_title}
                <li><a href="http://gpgtools.lighthouseapp.com/projects/{$lighthouse}/tickets/{$l_id}">Issue #439{$l_id}</a>: GPGMail is not compatible with 10.8. Look <a href="http://support.gpgtools.org/kb/faq/how-can-i-uninstall-your-software">here</a> for info, on how to remove GPGMail.</li>
                {/foreach}
            </ul>
            {else}
            <p><br/><br/></p>
